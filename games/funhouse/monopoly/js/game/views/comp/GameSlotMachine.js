...
export class GameSlotMachine extends SlotMachineV1 {
    constructor(config) {
        super(config);
        this.mysteryCloverAnimation = { name: "mysteryCloverAnimation", type: "Spine", spineName: "frame", defaultSkin: "default", defaultState: "animation", scale: 0.8, loop: false };
        this.wildMonopolyIndex = 0;
    ...
    showRandomWildOnReel() {
        ...
        for (let k = 0; k < rnd; k++) {
            ...
            this.addChild(sp);
            sp.scale.set(1);
            sp.x = this.symbolsArray[posArray[k].reel][posArray[k].row + 1].x + 162;
            sp.y = this.symbolsArray[posArray[k].reel][posArray[k].row + 1].y + 94;
            CoreLib.AnimationManager.animateTween(sp, 1, { alpha: 1, delay: k * 1.2, scaleX: 1.5, scaleY: 1.5 });
    ...
    showRandomMysteriesOnReel() {
        ...
        for (let k = 0; k < rnd; k++) {
            ...
            this.addChild(sp);
            sp.x = this.symbolsArray[posArray[k].reel][posArray[k].row + 1].x + 146;
            sp.y = this.symbolsArray[posArray[k].reel][posArray[k].row + 1].y + 105;
            CoreLib.AnimationManager.animateTween(sp, 1, { alpha: 1, delay: k * 1.2, scaleX: 1.5, scaleY: 1.5 });
            CoreLib.AnimationManager.animateTween(sp, 1, { alpha: 0, delay: k * 0.25, onComplete: this.clearSpecialElement.bind(this, sp) });
        }
    }
    ...
    showMysteryTransformSymbInSlot(data) {
        ...
        for (let k = 0; k < len; k++) {
            ...
            //play the clover spine anim
            const cloverSpine = CoreLib.UIUtil.getElement(this.mysteryCloverAnimation);
            this.winFrameContainer.addChild(cloverSpine);
            cloverSpine.playAnimation("animation", false);
            this.customCloverArray.push(cloverSpine);
            cloverSpine.x = this.symbolsArray[mysteryPositions[k].reel][mysteryPositions[k].row + 1].x;
            cloverSpine.y = this.symbolsArray[mysteryPositions[k].reel][mysteryPositions[k].row + 1].y;
        }
    }
    ...
    showWildPlacement(positions) {
        this.wildMonopolyIndex = 0;
        this.wildMonopolyData = positions;
        this.customSymbolsArray = [];
        this.showNextWild();
    }
    ...
    showNextWild() {
        ...
        let position = this.wildMonopolyData[this.wildMonopolyIndex];
    ...
    wildShowFirstStepCompleted(symb) {
        CoreLib.AnimationManager.animateTween(symb, 0.5, { scaleX: 1, scaleY: 1, onComplete: this.wildShowSecondStepCompleted.bind(this) });
    }

    wildShowSecondStepCompleted() {
        if (this.wildMonopolyIndex < this.wildMonopolyData.length - 1) {
            this.wildMonopolyIndex++;
    ...
    onWildPlacementDone() {
        CoreLib.EventHandler.dispatchEvent("MONOPOLY_WILD_PLACEMENT_DONE")
