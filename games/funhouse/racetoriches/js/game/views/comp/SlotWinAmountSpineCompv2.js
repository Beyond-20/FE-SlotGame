...
export class SlotWinAmountSpineCompV2 extends LibContainer {
    constructor(config) {
        ...
        this.luckOfTheTrisbAnim = this.elementsList["luckOfTheTrisbAnim"];
        ...
        if (this.luckOfTheTrisbAnim) {
            this.luckOfTheTrisbAnim.visible = false;
            CoreLib.UIUtil.setPosition(this.luckOfTheTrisbAnim, this.guideRect.width * 0.5, this.guideRect.height * CoreLib.Util.getDefaultValue(this.luckOfTheTrisbAnim.configData.yPaddingPerc, 0.25));
            this.luckOfTheTrisbAnim.addEventListener("complete", this.onSpineAnimComplete.bind(this));
        }
        //this.showWin(200, 0)
    ...
    showWin (val, level = 0, callback = null) {
        ...
        if (level == 1) {
            ...
            this.luckOfTheTrisbAnim.playAnimation(this.luckOfTheTrisbAnim.configData.defaultState, this.luckOfTheTrisbAnim.configData.loop != undefined ? this.luckOfTheTrisbAnim.configData.loop : true);
            delay = 5000;
        } else {
    ...
    onSpineAnimComplete (data) {
        ...
        if (this.configData.loopAfterEnd) {
            if (this.winLevel == 1) {
                ...
                this.luckOfTheTrisbAnim.scale.x = this.luckOfTheTrisbAnim.scale.y = CoreLib.Util.getDefaultValue(this.luckOfTheTrisbAnim.configData.scale, 1);
                CoreLib.AnimationManager.animateTween(this.luckOfTheTrisbAnim, 0.5, {repeat : -1, yoyo : true, scaleX : this.luckOfTheTrisbAnim.scale.x * 1.1, scaleY : this.luckOfTheTrisbAnim.scale.x * 1.1})
            } 
        } else {
            if (this.winLevel == 1) {
                this.bigAnim.scale.x = this.bigAnim.scale.y = CoreLib.Util.getDefaultValue(this.bigAnim.configData.scale, 1);
    ...
    cleanUpAllBigWins () {
        ...
        if (this.luckOfTheTrisbAnim) {
            this.luckOfTheTrisbAnim.visible = false;
            this.luckOfTheTrisbAnim.stopAnimation();
            this.luckOfTheTrisbAnim.scale.x = this.luckOfTheTrisbAnim.scale.y = CoreLib.Util.getDefaultValue(this.luckOfTheTrisbAnim.configData.scale, 1);
            CoreLib.AnimationManager.killTweensOf(this.luckOfTheTrisbAnim);
        }
    }
