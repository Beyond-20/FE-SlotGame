...
export class GameApp extends Application {
    ...
    createGameView() {
        ...
        CoreLib.EventHandler.addEventListener("MYSTERY_SYMBOLS_REPLACEMENT_DONE", this.showActualReelView.bind(this));
        ...
        CoreLib.EventHandler.addEventListener("ON_RED_JOCKEY_ANIMATION_DONE", this.onRedJockyAnimationDone.bind(this));
        CoreLib.EventHandler.addEventListener("RED_CROSSED_GREEN_POPUP_SHOWN_DONE", this.endFreespinRoundFromPopup.bind(this));
    ...
    checkForNextFreespin() {
        if (slotModel.isNextFreespinPossible()) {
            ...
            CoreLib.EventHandler.dispatchEvent("MAKE_RED_MAN_SLEEP");
            let data = slotModel.getFeatureData();
    ...
    startStoppingReels() {
        ...
        if (positions && positions.length > 0 && respinPosition.length == 0) {
            ...
            if (arr.length > 0) {
                CoreLib.EventHandler.dispatchEvent("INITIATE_JOCKY_WILD", arr);
            } else {
